...
//     · 入库台账 = 入出库明细页（Warehouse.vue，store key = guangwei-purchase）
//       的 receipts 子表 —— 即"广为仓库仓库实际接收入库量"。
//     · 出库台账 = 订单列表页（OrderList.vue，store key = guangwei-sales）的
//       shipments 子表 —— 即"从广为仓库仓库装车发出去客户的量"。
...
//   useContractStore('guangwei-purchase', () => [])，那么"采购"store 会
...
export function usePhosphoricAcidInventory(): PhosphoricAcidInventory {
  const totalIn = computed(() => {
    ...
    const purchases = peekContractStore<{ receipts: { qty?: number }[] }>('guangwei-purchase')
  ...
  const totalOut = computed(() => {
    ...
    const sales = peekContractStore<{ shipments: { qty?: number }[] }>('guangwei-sales')
    if (!sales) return 0
