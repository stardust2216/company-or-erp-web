<template>
  <div class="order-list">
    <div class="page-header">
      <div>
        <h1 class="page-title">华维食品添加剂 - 订单管理</h1>
        <p class="page-desc">硫磺原料贸易订单管理（对应Excel：硫磺销售合同统计表）- 贸易模式</p>
      </div>
      ...
      <el-form :model="filterForm" inline>
        ...
        <el-form-item label="产品类型">
          <el-select v-model="filterForm.productType" placeholder="全部产品" clearable>
            ...
            <el-option label="食品添加剂硫磺" value="food_sulfur" />
          </el-select>
        </el-form-item>
...
onMounted(() => {
  // 筛选华维食品添加剂的订单
  total.value = orderList.value.filter(o => o.subsidiaryId === 'huawei').length
})
...
const handleView = (row: any) => {
  router.push({ name: 'HuaweiOrderDetail', params: { id: row.id } })
}
