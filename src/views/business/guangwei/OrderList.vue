  ...
  <div class="sales-contracts-page" :class="{ 'is-fullscreen': isFullscreen, 'is-detail-route': isDetailRoute }">
    ...
    <header class="page-hero">
      <div class="hero-left">
        ...
        <div class="hero-info">
          <h1 class="hero-title">广为科技 - 订单列表</h1>
    ...
    <!--
      ...
      设计动机（迭代历史）：
        ...
        v4) 当前：保留全屏 el-drawer 形态，但不再走独立路由
            ...
            标签。GuangweiOrderListDetail 路由仍保留用于"直接通过 URL
            进入详情"的兼容场景：进入后会同步打开抽屉并把 URL 静默
            replace 回 GuangweiOrderList，避免历史栈污染。
      ===================================================================
    -->
    <el-drawer
...
//   · 列表路由（GuangweiOrderList）与每个详情路由
//     （GuangweiOrderListDetail/:id）实际上是同一份 SFC 的多个实例，
//     但 `<script setup>` 顶层的 ref 在每个实例里独立创建，会导致两边
//     看到的合同数据"互不联动"。
//   · useContractStore 把 ref 提升到模块级、跨实例共享，详情页所做的
//     任何修改（编辑合同 / 增删改 5 类登记）都会实时反映到列表标签页。
// ===========================================================
const contracts = useContractStore<Contract>('guangwei-sales', () => [
  {
    id: 'c-001',
    groupName: '南华集团',
    region: '海南',
  ...
  // ============= c-009 南圩糖厂 散单（原始表头行无合同号，仅记录单次散水交易） =============
  {
    ...
    contractPrice: 6900,
    shipments: [
      // 表格首行：2026.04.02 发货 13.06 吨，发自广为仓库
...
function consumeOpenIdFromQuery() {
  ...
  if (target) {
    ...
    router.replace({ name: 'GuangweiOrderList', query: cleaned })
    openDrawer(target)
  } else {
    ElMessage.warning('未找到对应合同，可能已被删除或筛选条件不同')
    router.replace({ name: 'GuangweiOrderList' })
  }
}
...
//     恢复列表上下文。
//   · 兼容旧 URL（GuangweiOrderListDetail/:id）：syncDetailFromRoute
 ...
 */
const isDetailRoute = computed(() => route.name === 'GuangweiOrderListDetail')
 ...
 *
 * 仅在用户通过 GuangweiOrderListDetail 路由直接进入时生效：
 *   1) 找到对应合同 → openDrawer 打开抽屉；
 *   2) 同时 router.replace 回 GuangweiOrderList，确保抽屉关闭后
 ...
 * 看板视图样式已迁出
 * 看板模板 / 样式整体迁移到通用子组件
 * `components/GuangweiBoardDrawer.vue`，主表不再持有相关 CSS。
 * ===================================================================== */
