<!--
  ============================================================================
  华维食品 · 硫磺、磷酸入出库流水视图（Plan B 形态 / 数据沿用合同子记录）
  ----------------------------------------------------------------------------
  设计原则：
  ...
  注意事项：
    - contractStore.useContractStore 在 store 已存在但为空时支持"种子补齐"，
      因此本组件即便先于 OrderList.vue 调用 useContractStore('huawei-sales')
      也不会"吞掉"销售合同的真实种子数据。
  ============================================================================
-->
<template>
...
function jumpToContract(row: FlowRow) {
  if (row.direction === 'in') {
    ...
    router.push({ name: 'HuaweiOrderList', query: { openId: row.contractId } })
  }
}
