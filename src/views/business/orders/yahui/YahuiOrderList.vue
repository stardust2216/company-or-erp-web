<template>
  <div class="order-list">
    <div class="page-header">
      <div>
        <h1 class="page-title">亚辉塑业 - 订单管理</h1>
        <p class="page-desc">编织袋、集装袋订单管理（对应Excel：亚辉塑业订单管理—统计表A/B/C）</p>
      </div>
      ...
      <el-form :model="filterForm" inline>
        ...
        <el-form-item label="产品类型">
          <el-select v-model="filterForm.productType" placeholder="全部产品" clearable>
            <el-option label="编织袋" value="weaving_bag" />
            <el-option label="集装袋" value="container_bag" />
            <el-option label="吨袋" value="ton_bag" />
          </el-select>
        </el-form-item>
...
onMounted(() => {
  // 筛选亚辉塑业的订单
  total.value = orderList.value.filter(o => o.subsidiaryId === 'yahui').length
})
...
const handleView = (row: any) => {
  router.push({ name: 'YahuiOrderDetail', params: { id: row.id } })
}
