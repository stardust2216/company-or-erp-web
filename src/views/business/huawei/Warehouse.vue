  ...
  <div class="purchase-contracts-page" :class="{ 'is-fullscreen': isFullscreen, 'is-detail-route': isDetailRoute }">
    ...
    <header class="page-hero">
      <div class="hero-left">
        ...
        <div class="hero-info">
          <h1 class="hero-title">华维食品 - 入出库明细</h1>
    ...
    <!--
      ...
      到 HuaweiWarehouseDetail，避免 TagsView 标签栏膨胀。详细设计
...
// ===========================================================
const contracts = useContractStore<Contract>('huawei-purchase', () => [
  // ===========================================================================
...
function consumeOpenIdFromQuery() {
  ...
  if (target) {
    ...
    router.replace({ name: 'HuaweiWarehouse', query: cleaned })
    openDrawer(target)
  } else {
    ElMessage.warning('未找到对应合同，可能已被删除或筛选条件不同')
    router.replace({ name: 'HuaweiWarehouse' })
  }
}
...
// drawerVisible=true，不再 router.push 到 HuaweiWarehouseDetail，
// 因此不会向 TagsView 追加新标签页。HuaweiWarehouseDetail 路由
...
/** 兼容旧 URL：用户通过 HuaweiWarehouseDetail 路由直接进入时为 true。 */
const isDetailRoute = computed(() => route.name === 'HuaweiWarehouseDetail')
 ...
 *
 * 仅在 HuaweiWarehouseDetail 路由命中时生效：找到合同 → 打开
 ...
 * 通用子组件 `components/HuaweiBoardDrawer.vue`，主表不再持有相关 CSS。
 * ===================================================================== */
...
/*
 * 华维采购详情·全屏抽屉样式（非 scoped，与 OrderList.vue 同款）
