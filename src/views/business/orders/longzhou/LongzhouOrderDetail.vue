<template>
  <div class="page-container">
    <el-page-header @back="$router.back()">
      <template #content>
        <span class="page-header-title">龙州塑业 - 订单详情</span>
      </template>
    </el-page-header>
    ...
    <div v-if="order" class="page-body">
      <div class="toolbar no-print">
        ...
        <el-button @click="router.push({ name: 'LongzhouOrders' })">返回列表</el-button>
      </div>

      <section class="block">
        <h3 class="block-title">订单基本信息</h3>
        <el-descriptions :column="2" border size="default">
          <el-descriptions-item label="订单号">{{ order.orderNo }}</el-descriptions-item>
          <el-descriptions-item label="子公司">龙州塑业</el-descriptions-item>
    ...
    <div v-else class="page-body empty-wrap">
      <el-empty description="未找到该订单（可能已删除）">
        <el-button type="primary" @click="router.push({ name: 'LongzhouOrders' })">返回订单列表</el-button>
      </el-empty>
    </div>
  </div>
</template>
